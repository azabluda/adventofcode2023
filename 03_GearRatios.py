...
def gear_ratios(data):
    def scan(pattern, nei = 0):
        for i, s in enumerate(data.split('\n')):
            for m in finditer(pattern, s):
                l, r = m.start(), m.end()
                yield m[0], [*product(range(i - nei, i + nei + 1), range(l - nei, r + nei))]

    # part 1
    symbols = {s[0] for _, s in scan(r'[^\d\.]')}
    yield sum(int(m) for m, s in scan(r'\d+', 1) if symbols & {*s})

    # part 2
    stars = {s[0] for _, s in scan(r'\*')}
    lkp = defaultdict(list)
    for m, s in scan(r'\d+', 1):
        for star in stars & {*s}:
            lkp[star] += int(m),
    yield sum(lst[0] * lst[1] for lst in lkp.values() if len(lst) == 2)
