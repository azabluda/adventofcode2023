...
def haunted_wasteland(data):
    lines = data.split('\n')
    moves = {x[0:3]: (x[7:10], x[12:15]) for x in lines[2:]}
    def solv(cur, end):
        seq = iter(cycle(lines[0]))
        for steps in count():
            if end(cur): return steps
            cur = moves[cur][next(seq) == 'R']
    yield solv('AAA', 'ZZZ'.__eq__)
    ghosts = (solv(m, lambda x: x[2] == 'Z') for m in moves if m[2] == 'A')
    yield reduce(lcm, ghosts)
...
for data in inputs():
    print(*haunted_wasteland(data.strip()))
