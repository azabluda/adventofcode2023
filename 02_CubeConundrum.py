...
def cube_conundrum(data):
    ...
    res1 = res2 = 0
    valid_game = Counter(red=12, green=13, blue=14)
    for id, line in enumerate(data.split('\n'), 1):
        record = line.split(': ')[1]
        game = Counter()
        for n_cube in record.replace(';', ',').split(', '):
            n, cube = n_cube.split(' ')
            game |= {cube: int(n)}
        res1 += game <= valid_game and id
        res2 += prod(game.values())
    return res1, res2
...
for data in inputs():
    print(*cube_conundrum(data.strip()))
